    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div ref={proposalRef} className="max-w-6xl mx-auto bg-white">
        ...
        <div className="px-8 pb-8 space-y-12">
          ...
          {/* Understanding Company — from transcript */}
          {proposal.personalizedContent && proposal.personalizedContent.keyChallenges.length > 0 && (
            <section data-pdf-block>
              ...
              {proposal.personalizedContent.companySituation && (
                <p className="text-gray-700 leading-relaxed text-lg mb-6">{proposal.personalizedContent.companySituation}</p>
              )}
              {(() => {
                ...
                const grouped = agentOrder
                  .map(a => ({ agent: a, items: pc.keyChallenges.filter(c => c.agent === a) }))
                  .filter(g => g.items.length > 0);

                return grouped.map((group, gi) => (
                  <div key={gi} className="mb-6">
                    <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">{AGENT_LABELS[group.agent]}</h3>
                    <div className="space-y-3">
                      {group.items.map((item, i) => (
                        <div key={i} className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                          <div className="flex items-start mb-2">
                            <div className="flex-shrink-0 w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center mt-0.5 mr-3">
                              <span className="text-white text-xs font-bold">!</span>
                            </div>
                            <p className="text-gray-800 font-medium text-sm">{item.challenge}</p>
                          </div>
                          <div className="flex items-start">
                            <div className="flex-shrink-0 w-6 h-6 bg-green-600 rounded-full flex items-center justify-center mt-0.5 mr-3">
                              <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                              </svg>
                            </div>
                            <p className="text-gray-700 text-sm">{item.megaSolution}</p>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="bg-blue-50 rounded-lg p-6 space-y-4">
                {proposal.personalizedContent.specificGoals.map((goal, i) => (
                  <div key={i} className="flex items-start">
                    ...
                    <p className="text-gray-800 font-medium text-sm">{goal}</p>
                  </div>
                ))}
              </div>
            </section>
